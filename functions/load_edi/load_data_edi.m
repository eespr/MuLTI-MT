...
%
% Usage: [d,units_code] = load_data_edi(filename)
%
% INPUTS
%
...
% Zerr = impedance standard error in SI units
...
while 1
    ...
    % --- NEW CODE BLOCK FOR TIPMAG/TIPPHS/TSTRIKE/TELLIP ---
    if strncmp(strtrim(tline),'>TIPMAG',7) && ~strncmp(strtrim(tline),'>TIPMAG.ERR',11)
        nfreq=find_nfreq(tline);
        tipmag=fscanf(fid,'%f',nfreq);
    end
    if strncmp(strtrim(tline),'>TIPPHS',7) && ~strncmp(strtrim(tline),'>TIPPHS.ERR',11)
        nfreq=find_nfreq(tline);
        tipphs=fscanf(fid,'%f',nfreq);
    end
    if strncmp(strtrim(tline),'>TSTRIKE',8)
        nfreq=find_nfreq(tline);
        tstrike=fscanf(fid,'%f',nfreq);
    end
    if strncmp(strtrim(tline),'>TELLIP',7)
        nfreq=find_nfreq(tline);
        tellip=fscanf(fid,'%f',nfreq);
    end
    if strncmp(strtrim(tline),'>TIPMAG.ERR',11)
        nfreq=find_nfreq(tline);
        tipmagerr=fscanf(fid,'%f',nfreq);
    end
    % ----------------------------------------
...
% --- UPDATED TROT INITIALIZATION ---
% Default to zrot if trot is missing, instead of NaN
if ~exist('trot','var'); trot=zrot; end
...
% --- MODIFIED TIPPER LOGIC START (Includes 3D Ellipse Reconstruction) ---
% First, try the standard TXR/TXI format
if exist('txr','var')
    T(1,:)=txr(1:length(f))+(1i.*txi(1:length(f))); 
end 
if exist('tyr','var')
    T(2,:)=tyr(1:length(f))+(1i.*tyi(1:length(f))); 
end

% If standard format not found, look for TIPMAG/TIPPHS
if ~exist('txr','var') && ~exist('tyr','var') && exist('tipmag','var') && exist('tipphs','var')
    % Check for existence of TipMag and TipPhase variables
    % Convert phase from degrees to radians
    d_rad_phs = tipphs(1:length(f)) .* (pi/180);
    
    % Initialize T_total magnitude (Major Axis 'a' if strictly 2D, Total Mag 'M' if 3D)
    % Standard usually implies TIPMAG = sqrt(|Tx|^2 + |Ty|^2)
    M = tipmag(1:length(f));
    
    % Check for TSTRIKE (Orientation)
    if exist('tstrike','var')
        d_rad_strike = tstrike(1:length(f)) .* (pi/180);
    else
        warning('EDI Loading: TIPMAG found but no TSTRIKE. Assuming North alignment.');
        d_rad_strike = zeros(size(M));
    end

    % Check for TELLIP (Ellipticity) to handle 3D structures
    if exist('tellip','var')
        epsilon = tellip(1:length(f));
        % Calculate Major (a) and Minor (b) axis magnitudes
        % Relation: M^2 = a^2 + b^2  and  epsilon = b/a
        a = M ./ sqrt(1 + epsilon.^2);
        b = a .* epsilon;
        
        % Reconstruct using Major Axis (Real) and Minor Axis (Imaginary/90deg rotated)
        % The Minor axis is perpendicular to Strike (-sin, cos) and has '1i' factor
        Tx_calc = (a .* cos(d_rad_strike) - 1i .* b .* sin(d_rad_strike)) .* exp(1i .* d_rad_phs);
        Ty_calc = (a .* sin(d_rad_strike) + 1i .* b .* cos(d_rad_strike)) .* exp(1i .* d_rad_phs);
    else
        % Fallback to 2D assumption if TELLIP is missing (Minor axis = 0)
        warning('EDI Loading: TELLIP not found. Assuming 2D Earth (Linear Polarization).');
        T_total = M .* exp(1i .* d_rad_phs);
        Tx_calc = T_total .* cos(d_rad_strike);
        Ty_calc = T_total .* sin(d_rad_strike);
    end
        
    % FIX: Transpose results to match dimensions of T (Row Vectors)
    T(1,:) = Tx_calc.';
    T(2,:) = Ty_calc.';
    
    % Handle Error if present (TIPMAG.ERR)
    if exist('tipmagerr','var')
        % Converting Sigma to Variance for consistency with script format
        % Transposed to match Tvar dimensions
        Tvar(1,:) = tipmagerr(1:length(f)).^2.';
        Tvar(2,:) = tipmagerr(1:length(f)).^2.';
    end
end
% --- MODIFIED TIPPER LOGIC END ---
